function [t_ext, TEMP_ext, niter] = psr_extinction_T(gas,P_in, T_in, COMP, y_ext0, T_ext0) 
    % ///////////////////////////////////////////////////////////////////
...
%     yin = [massFractions(gas); enthalpy_mass(gas)];
    ...
    y1 = func_unsteady_psr(gas, y0, Omega_init, T_ext0);
    toc;
%     
...
function y = func_unsteady_psr(gas, y0, T_ext0)
    % function for computing unsteady psr
    global nsp;
    options = odeset('RelTol',1.e-5,'AbsTol',1.e-12,'Stats','off');
    tel = [0 20/Omega];
    out = ode15s(@func_psr_ode,tel,y0,options, gas, T_ext0); %pass extra paras
    y = out.y(:,end);
%     y = [Omega; y; temphy(gas, y(1:nsp+1))];
end

function dydt = func_psr_ode( t, y, gas, T_ext0) %#ok<INUSL>
